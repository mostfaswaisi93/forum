...
const mutations = {
    fetchChannelsSuccess(state, {
        channels
    }) {
        state.channels = channels;
    },
    fetchDiscussionsSuccess(state, {
        discussions
    }) {
        state.discussions = discussions;
    },
    fetchDiscussionSuccess(state, {
        discussion
    }) {
...
const actions = {
    ...
    async fetchDiscussions({
        ...
        try {
            const {
                data
            } = await axios.get("/api/v1/discussions")
    ...
    async fetchDiscussion({
        ...
        try {
            const {
                data
            } = await axios.get(`/api/v1/discussions/${slug}`)
